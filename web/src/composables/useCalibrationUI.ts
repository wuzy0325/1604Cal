...
export function useCalibrationUI() {
  ...
  const connectDevice1604 = async (id: string) => handleResult(() => store.connectDevice1604(id), '')
  const disconnectDevice1604 = async (id: string) => handleResult(() => store.disconnectDevice1604(id), '')
